...
class MainWindow(wx.Frame):
    ...
    def __init__(self, parent, id, title):
        ...
        ## moj dodatni modul za delta RR
        self.buttonDiffRR = wx.Button(self.MainPanel, -1, label="Delta RR")
        sbToolsButtonsSizer.Add(self.buttonDiffRR, flag=wx.ALL | wx.EXPAND, border=borderSmall)
        #self.Bind(wx.EVT_BUTTON, self.OnDiffRR, id=self.buttonDiffRR.GetId())
        self.Bind(wx.EVT_BUTTON, self.OnDiffRR, id=self.buttonDiffRR.GetId())
        self.buttonDiffRR.SetToolTip(wx.ToolTip("Delta RR tool"))
        if platform != 'darwin' and ColoredButtons:
            self.buttonDiffRR.SetBackgroundColour(TemporalBGColor)
        self.buttonDiffRR.Disable()
...
#            self.RefreshMainWindowButtons()
    ...
    def DisableAllButtons(self):
        ...
        self.buttonTemporal.Disable()
        self.buttonPoincare.Disable()
        self.buttonDiffRR.Disable() #dodat button za diffRR
    ...
    def OnPoincareEnded(self):
        ...
        ######### dotati modul za RR - implementirati
    def OnDiffRR(self,event): #def OnDiffRR(self,event):
        #dm.ClearPP()
        varijabla=dm.GetHR_RR()
        varijabla=np.diff(varijabla)
        import matplotlib.pyplot as plt
        plt.plot(varijabla)
        plt.xlabel('index (n)')
        plt.ylabel('$\Delta RR (ms)$')
        plt.title('$\Delta$RR interval data')
        plt.show()

        #varijablat=varijabla.size
        #plot(varijabla)
        #show()
        #PoincarePlotWindow(self,-1,'Poincare plot',dm)
        #self.poincareWindowPresent=True
   ...
   # def OnDiffRREnded():  #def OnDiffRREnded(self):
       # self.poincareWindowPresent=False
       # self.RefreshMainWindowButtons()
       #kraj modula za RR
